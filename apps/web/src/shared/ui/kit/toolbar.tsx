/** biome-ignore-all lint/a11y/useFocusableInteractive: Library needs */
/** biome-ignore-all lint/a11y/useSemanticElements: Library needs */
"use client";
...
import { DropdownMenuLabel, DropdownMenuRadioGroup, DropdownMenuSeparator } from "@/shared/ui/kit/dropdown-menu";
import { Separator } from "@/shared/ui/kit/separator";
import { Tooltip, TooltipTrigger } from "@/shared/ui/kit/tooltip";
...
function withTooltip<T extends React.ElementType>(Component: T) {
    return function ExtendComponent({ tooltip, tooltipContentProps, tooltipProps, tooltipTriggerProps, ...props }: TooltipProps<T>) {
...
export function ToolbarMenuGroup({ children, className, label, ...props }: React.ComponentProps<typeof DropdownMenuRadioGroup> & { label?: string }) {
    return (
        <>
            ...
            >
                {label && <DropdownMenuLabel className="select-none font-semibold text-muted-foreground text-xs">{label}</DropdownMenuLabel>}
                {children}
            </DropdownMenuRadioGroup>
        </>
    );
