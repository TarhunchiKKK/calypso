...
export class ResizableArrowStrategy extends ResizableNodeStrategy {
    public override ui(node: Decoratable<ArrowNode>,handler: ResizeHandler) {
        ...
        return (
            <>
                <div className={className} style={{ left: start.x, top: start.y }} onMouseDown={() => handler(node.id, "s")} />

                <div className={className} style={{ left: end.x, top: end.y }} onMouseDown={() => handler(node.id, "n")} />
            </>
        );
    }
}
